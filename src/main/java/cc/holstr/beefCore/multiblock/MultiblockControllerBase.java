...
import cc.holstr.helper.WorldHelper;
...
public abstract class MultiblockControllerBase {
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if(minimumCoord != null) {
			...
			if(pos.getX() < minimumCoord.getX()) { x = pos.getX(); }
			if(pos.getY() < minimumCoord.getY()) { y = pos.getY(); }
			if(pos.getZ() < minimumCoord.getZ()) { z = pos.getZ(); }
			
			minimumCoord.setPos(x,y,z);
		}

		if(maximumCoord != null) {
			int x = maximumCoord.getX();
			int y = maximumCoord.getY();
			int z = maximumCoord.getZ();
			if(pos.getX() > maximumCoord.getX()) { x = pos.getX(); }
			if(pos.getY() > maximumCoord.getY()) { y = pos.getY(); }
			if(pos.getZ() > maximumCoord.getZ()) { z = pos.getZ(); }

			maximumCoord.setPos(x,y,z);
		}
	...
	private void _onAssimilated(MultiblockControllerBase otherController) {
		if(referenceCoord != null) {
			if(worldObj.getChunkProvider().isChunkGeneratedAt(BlockPosHelper.getChunkX(referenceCoord), BlockPosHelper.getChunkZ(referenceCoord))) {
				TileEntity te = this.worldObj.getTileEntity(referenceCoord);
	...
	public final void updateMultiblockEntity() {
		...
		if(worldObj.isRemote) {
			...
			if(minimumCoord != null && maximumCoord != null &&
					 WorldHelper.checkChunksExist(this.worldObj, minimumCoord, maximumCoord)) {
	...
	public void recalculateMinMaxCoords() {
		minimumCoord = new BlockPos.MutableBlockPos(Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE);
		maximumCoord = new BlockPos.MutableBlockPos(Integer.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE);
		...
		for(IMultiblockPart part : connectedParts) {
			...
			if(pos.getX() < minimumCoord.getX()) { minX = pos.getX(); }
			if(pos.getX() > maximumCoord.getX()) { maxX = pos.getX(); }
			if(pos.getY() < minimumCoord.getY()) { minY = pos.getY(); }
			if(pos.getY() > maximumCoord.getY()) { maxY = pos.getY(); }
			if(pos.getZ() < minimumCoord.getZ()) { minZ = pos.getZ(); }
			if(pos.getZ() > maximumCoord.getZ()) { maxZ = pos.getZ(); }

			minimumCoord.setPos(minX,minY,minZ);
			maximumCoord.setPos(maxX,maxY,maxZ);
		}
	}

	//todo: originally these returned a new instance of min & max coord...
	/**
	 * @return The minimum bounding-box coordinate containing this machine's blocks.
	 */
	public BlockPos.MutableBlockPos getMinimumCoord() {
	 ...
	 */
	public BlockPos.MutableBlockPos getMaximumCoord() {
	...
	private String getPartsListString() {
		...
		for(IMultiblockPart part : connectedParts) {
			...
			BlockPos pos = part.getPos();
			sb.append(String.format("(%d: %d, %d, %d)", part.hashCode(), pos.getX(), pos.getY(), pos.getZ()));
			first = false;
		}
	...
	public Set<IMultiblockPart> checkForDisconnections() {
		...
		for(IMultiblockPart part : connectedParts) {
			...
			if(!chunkProvider.isChunkGeneratedAt(part.getPos().getX() >> 4, part.getPos().getZ() >> 4) || part.isInvalid()) {
				deadParts.add(part);
				onDetachBlock(part);
				continue;
			}
	...
	public Set<IMultiblockPart> detachAllBlocks() {
		...
		IChunkProvider chunkProvider = worldObj.getChunkProvider();
		for(IMultiblockPart part : connectedParts) {
			if(chunkProvider.isChunkGeneratedAt(part.getPos().getX() >> 4, part.getPos().getZ() >> 4)) {
				onDetachBlock(part);
			}
		}
	...
	private void selectNewReferenceCoord() {
		...
		for(IMultiblockPart part : connectedParts) {
			if(part.isInvalid() || !chunkProvider.isChunkGeneratedAt(part.getPos().getX() >> 4, part.getPos().getZ() >> 4)) {
