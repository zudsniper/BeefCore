...
import cc.holstr.helper.ChunkHelper;
...
public class MultiblockWorldRegistry {
		...
		if(orphanedParts.size() > 0) {
			...
			if(orphansToProcess != null && orphansToProcess.size() > 0) {
				...
				for(IMultiblockPart orphan : orphansToProcess) {
					...
					if(!chunkProvider.isChunkGeneratedAt(BlockPosHelper.getChunkX(coord), BlockPosHelper.getChunkZ(coord))) {
						continue;
					}
	...
	public void onPartAdded(IMultiblockPart part) {
		...
		ChunkPos chunkLocation = worldObj.getChunkFromBlockCoords(worldLocation).getPos();

		if(!worldObj.getChunkProvider().isChunkGeneratedAt(chunkLocation.x, chunkLocation.z)) {
			// Part goes into the waiting-for-chunk-load list
			Set<IMultiblockPart> partSet;
			long chunkHash = ChunkHelper.getChunkXZHash(chunkLocation);
	...
	public void onPartRemovedFromWorld(IMultiblockPart part) {
		...
		ChunkPos chunkLocation = worldObj.getChunkFromBlockCoords(coord).getPos();

		if(coord != null) {
			long hash = ChunkHelper.getChunkXZHash(chunkLocation);
